...
The monitor uses the existing `objc2-app-kit` bridge and declares `block2` directly because AppKit's local event-monitor API accepts an Objective-C block.

## Options considered

- **Native conditional interception with renderer surface state** (chosen): preserves standard macOS fullscreen Escape when no overlay is open while giving dialogs first and exclusive handling.
