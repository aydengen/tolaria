...
The renderer keeps app shortcuts platform-exact where they overlap with native desktop affordances. Note-list search treats `Cmd+F` on macOS and `Ctrl+F` elsewhere as the pane search toggle, but leaves macOS `Cmd+Ctrl+F` untouched so the predefined fullscreen item in the native Window menu owns it. The main entrypoint also owns macOS window-chrome body state: `mac-chrome` enables traffic-light offsets and `mac-chrome-fullscreen` lowers the shared traffic-light padding CSS variable when native fullscreen hides those controls. Visible shared dialogs, popovers, and the legacy Settings panel register their lifetime with the native AppKit Escape monitor; in fullscreen, the monitor consumes the first physical Escape and redispatches it to the renderer so the surface closes, while a subsequent bare Escape remains native and exits fullscreen.
...
Warm startup entry hydration calls `read_vault_snapshot` for the active workspace before `list_vault`. A snapshot is a provisional, version- and path-valid cache read: it must not invoke Git, stat each cached file, sort entries, or replace the cache. React may clear its blocking loading state after installing this graph, but it must start `list_vault` reconciliation immediately and apply that workspace's authoritative result when ready. Missing/invalid snapshots, forced reloads, and Gitignored-hidden mode fall back to the scan path. Secondary workspaces hydrate serially after the active workspace; when a mounted path is inside a loaded ancestor, `vaultWorkspaceEntries` retags the ancestor-supplied subtree instead of requesting a duplicate scan. Known watcher paths still refresh with `reload_vault_entry`; explicit user reloads, Git pull refreshes, and watcher paths that cannot be reloaded individually use full reload paths.

`LazyEditor` is the startup boundary for BlockNote, KaTeX, editor extensions, and editor-only CSS. It mounts a neutral editor surface for the first app-shell commit and only then requests the editor chunk. Startup milestone ownership lives in `startupPerformance.ts`: frontend timings are relative to renderer module initialization, native elapsed time comes from `StartupTimingState`, and analytics payloads may contain timing, entry counts, source, and target status but never paths or note data.
...
- `codeBlockLineNumbers.ts` maintains a non-editable gutter beside BlockNote's `pre > code` content DOM. It derives logical line numbers without inserting text into the note, positions each number from that line's rendered start instead of accumulating wrapped-row estimates, clips the gutter to the code content box, and refreshes after editor mutations or width changes.
...
The Quick Launcher renderer treats `(vaultId, relativePath)` as note identity and delegates cross-vault opening to the main window through the existing deep-link resolver. Its result list reuses `NoteSearchList` plus full vault-entry metadata, so note icons, type icons/colors, and workspace badges stay identical to Cmd+O. The native Tauri run loop owns focus-loss dismissal for the `quick-launcher` window so outside-click behavior does not depend on the renderer mounting or subscribing successfully; the renderer owns Escape dismissal. Every non-empty input remains available as a create-note action below the results, including when matches exist. Capture destinations use the pure `resolveQuickCaptureDestination` precedence rule (configured, active, last successful), but a configured unavailable vault is an attention state rather than permission to silently fall back. The launcher exposes only a bottom vault selector and always creates at the selected vault root. The input becomes the note H1 and filename stem; `create_quick_launcher_note` owns slugging, collision suffixes, vault-boundary validation, and the create-only write as one native operation so renderer lifecycle failures cannot be confused with failed persistence.
...
Quick Launcher shortcut and capture defaults are installation-local; per-vault global-search inclusion is stored with the vault registry and defaults on. Shortcut replacement preserves the previous registered chord on conflict and exposes retry/reset feedback in Settings.
